...
const ProductDetailPage = () => {
    ...
    // Rasm kelishi bilan active rasmga o'rnatish
    useEffect(() => {
        if (product?.image) {
    ...
    // ── LOADING WITH FULL PAGE SKELETON ──
    if (loading) return (
        ...
        >
            ...
            <div className="max-w-[1600px] mx-auto px-6 lg:px-16 py-10 lg:py-14 font-roboto">
                    ...
                    <div className="lg:col-span-5 lg:sticky lg:top-32 space-y-8">
                        ...
                        <div className="relative overflow-hidden border border-gray-100 bg-white p-6 sm:p-10 shadow-lg shadow-blue-50"
                            ...
                            <div className="relative z-10 space-y-8">
                                <div>
                                    <p className=" text-gray-400 text-[10px] tracking-widest uppercase font-bold mb-2">{t('price')}</p>
                                    <p className="text-3xl sm:text-4xl font-bold text-slate-900 tracking-tighter tabular-nums">
                                        {product.price ? `${product.price} ${lang === 'ru' ? 'сум' : "so'm"}` : t('call_price')}
                                    </p>
                                </div>
                                <button onClick={() => navigate('/contacts')} className="relative w-full flex items-center justify-center gap-3 py-5 sm:py-6 bg-[#0061A4] text-white font-bold text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all active:scale-[0.98] rounded-sm">
                                    <PhoneCall size={16} /> {t('contact_us')} <ArrowUpRight size={16} />
                                </button>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {product.advantages.map((adv) => (
                                    <div key={adv.id} className="relative flex items-center gap-3 px-4 py-4 bg-white border border-gray-100 hover:border-blue-500/30 hover:bg-blue-50/20 transition-all duration-300 group">
                                        ...
                                        {adv.icon ? <img src={`${API_URL}${adv.icon}`} className="w-6 h-6 object-contain shrink-0" alt="" /> : <ShieldCheck size={18} className="text-gray-300 group-hover:text-[#0061A4]" />}
                        ...
                        <div className="relative overflow-hidden p-8" style={{ background: 'linear-gradient(135deg, rgba(0,97,164,0.1) 0%, rgba(255,255,255,1) 100%)', border: '1px solid #E5E7EB' }}>
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-gray-400 text-xs sm:text-sm leading-relaxed mb-6 font-medium">{t('custom_solution_desc')}</p>
                                <button onClick={() => navigate('/contacts')} className="w-full py-4 bg-[#0061A4] text-white text-[11px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-200">{t('contact_us')}</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {specSections.length > 0 && (
                    <div className="mt-20 lg:mt-32 pt-16 border-t border-gray-100 font-roboto">
                        <h2 className="text-[36px] sm:text-[42px] lg:text-[56px] font-bold uppercase mb-12">
                                    ...
                                    <div className="px-6 sm:px-8 py-4">
                                        {section.specs.map((spec, si) => (
                                            <div key={spec.id} className={cn("spec-row flex justify-between items-end gap-4 py-3.5", si < section.specs.length - 1 ? "border-b border-gray-50" : "")}>
                                                <span className="spec-key text-gray-800 text-[10px] sm:text-[11px] opacity-70">{getField(spec, 'key')}</span>
                                                <span className="spec-val text-[#1A1C1E] font-bold text-[11px] sm:text-[12px] text-right">{getField(spec, 'val')}</span>
                                            </div>
                                        ))}
                                    </div>
                                </motion.div>
            ...
            <section className="mt-20 lg:mt-32 relative overflow-hidden font-roboto" style={{ backgroundColor: '#0061A4' }}>
                <div className="relative z-10 max-w-[1600px] mx-auto px-6 lg:px-16 py-20 lg:py-24 flex flex-col lg:flex-row lg:items-center justify-between gap-12 text-white">
                    <div>
                        <h2 className="text-[32px] sm:text-[42px] lg:text-[60px] font-bold uppercase mb-6">{lang === 'ru' ? 'ЗАИНТЕРЕСОВАНЫ?' : "QIZIQTIRDIMi?"}</h2>
                        <p className="text-blue-100/60 text-lg max-w-md">{lang === 'ru' ? 'Наши менеджеры ответят на все ваши вопросы в кратчайшие сроки.' : "Menejerlarimiz yordam berishga tayyor."}</p>
                    </div>
                    <button onClick={() => navigate('/contacts')} className="relative px-12 py-7 bg-white text-[#0061A4] font-bold text-[11px] uppercase tracking-widest hover:bg-blue-50 transition-all rounded-sm">

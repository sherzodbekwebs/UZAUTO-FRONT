    ...
    return (
        <div className="min-h-screen text-[#1A1C1E] font-['Barlow_Condensed',_sans-serif]"
            ...
            <section ref={heroRef} className="relative overflow-hidden border-b border-gray-100" style={{ minHeight: 340, backgroundColor: '#fcfdfe' }}>
                ...
                <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-blue-500/0 via-blue-500/20 to-blue-500/0" />
                ...
                <div className="relative z-10 max-w-[1600px] mx-auto px-6 lg:px-16 pt-20 pb-14">
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                        <div>
                            ...
                            <h1 className="text-[64px] lg:text-[92px] font-bold uppercase leading-none tracking-tight">
                                <GlitchText><span className="text-[#1A1C1E]">{t('products') || 'PRODUCTS'}</span></GlitchText>
                                <br />
                    ...
                    <div className="flex items-center gap-8 mt-10 pt-6 border-t border-gray-100">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-400 text-xs uppercase tracking-widest">Units found</span>
                        </div>
            ...
            <div className="max-w-[1600px] mx-auto px-6 lg:px-16 py-14">
                <div className="flex gap-12 items-start">
                    {/* 🛠️ ASIDE: KENGAYTIRILDI (320px) VA BORDER QO'SHILDI */}
                    <aside className={cn(
                        "lg:w-[320px] shrink-0 sticky top-6 z-50 lg:border-r lg:border-gray-100 lg:pr-12",
                        "max-lg:fixed max-lg:inset-y-0 max-lg:left-0 max-lg:w-[300px] max-lg:bg-white max-lg:overflow-y-auto max-lg:p-6 max-lg:transition-transform max-lg:duration-500",
                        sidebarOpen ? "max-lg:translate-x-0" : "max-lg:-translate-x-full"
                    )}>
                        <div className="flex items-center justify-between mb-8 lg:hidden">
                            <span className="mono text-[#0061A4] text-xs tracking-widest uppercase">Filters</span>
                            <button onClick={() => setSidebarOpen(false)} className="text-gray-400 hover:text-black transition-colors"><X size={20} /></button>
                        </div>

                        {/* 🛠️ CATEGORY LABEL: BORDER-T VA BORDER-B BILAN AJRATILDI */}
                        <div className="flex items-center gap-3 mb-6 py-4 border-y border-gray-50">
                            <div className="w-4 h-px bg-blue-500" />
                            <span className="mono text-gray-400 text-[10px] tracking-[0.3em] uppercase">// CATEGORY</span>
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            <button onClick={() => { setSearchParams({ category: 'all', brand: activeBrand }); setSidebarOpen(false); }}
                                ...
                                <div className={cn("absolute inset-0 transition-all duration-300", activeCategory === 'all' ? "bg-blue-50/50 border border-blue-100/50" : "group-hover:bg-gray-50")} />
                                <div className="relative flex items-center gap-4">
                                    <div className={cn("w-9 h-9 flex items-center justify-center border transition-all duration-300", activeCategory === 'all' ? "border-blue-500/50 text-[#0061A4]" : "border-gray-100 text-gray-300")}><Box size={16} /></div>
                                    <span className="text-[13px] font-bold uppercase tracking-wider">{t('all') || 'All Models'}</span>
                                </div>
                                <div className="relative flex items-center gap-2"><CountBadge count={products.length} /></div>
                            </button>
                            ...
                            {categories.map((cat, i) => {
                                ...
                                return (
                                    <button key={cat.id} onClick={() => { setSearchParams({ category: cat.id, brand: activeBrand }); setSidebarOpen(false); }}
                                        ...
                                        <div className={cn("absolute inset-0 transition-all duration-300", isActive ? "bg-blue-50/50 border border-blue-100/50" : "group-hover:bg-gray-50")} />
                                        ...
                                        <div className="relative flex items-center gap-2 shrink-0"><CountBadge count={count} /></div>
                                    </button>
                                );
                            })}
                        </div>
                    ...
                    <main className="flex-1 min-w-0">
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-5"><Loader2 className="text-[#0061A4] animate-spin" size={32} /><p className="mono text-gray-400 text-[11px] tracking-[0.3em] uppercase">LOADING FLEET DATA</p></div>
                        ) : filteredProducts.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-4"><Box size={48} className="text-gray-100" /><p className="mono text-gray-300 text-sm uppercase tracking-widest">NO UNITS FOUND</p></div>
                        ) : (
                            <AnimatePresence mode="popLayout">
                                <div className="flex flex-col gap-6">
                                    {filteredProducts.map((p, idx) => (
                                        <motion.article key={p.id} layout initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -40, scale: 0.97 }} transition={{ duration: 0.5, delay: idx * 0.06, ease: [0.16, 1, 0.3, 1] }} onClick={() => navigate(`/product/${p.id}`)} onHoverStart={() => setHoveredId(p.id)} onHoverEnd={() => setHoveredId(null)}
                                            className="product-card group relative cursor-pointer overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all">
                                            ...
                                            <div className="flex flex-col md:flex-row gap-0">
                                                <div className="relative md:w-[380px] shrink-0 overflow-hidden bg-gray-50 flex items-center justify-center p-8 border-r border-gray-50">
                                                    ...
                                                    {!p.inStock && <div className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-1.5 bg-red-500 text-white text-[9px] font-black uppercase tracking-widest shadow-lg rounded-sm">OUT OF STOCK</div>}
                                                </div>

                                                <div className="flex-1 flex flex-col p-8 lg:p-10 min-w-0">
                                                    <div className="mb-6">
                                                        <div className="flex items-center gap-3 mb-2">
                                                            <span className="mono text-[#0061A4]/80 text-[10px] font-bold uppercase tracking-[0.3em]">{p.brand?.name}</span>
                                                    ...
                                                    <div className="flex items-center justify-between mt-auto pt-6 border-t border-gray-50">
                                                        <div className="space-y-0.5">
                                                            <p className="mono text-slate-500 text-[10px] tracking-widest uppercase font-bold">{t('price')}</p>
                                                            <p className="text-2xl font-bold text-slate-900 tracking-tighter tabular-nums italic">
                                                                {p.price ? `${p.price} so'm` : '— CALL —'}
                                                            </p>
                                                        </div>
                                                        <button className="relative flex items-center gap-3 px-10 py-5 bg-[#0061A4] text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all hover:bg-blue-600 shadow-xl shadow-blue-100">

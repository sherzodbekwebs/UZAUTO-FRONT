...
const ProductsPage = () => {
    ...
    return (
        <div className="min-h-screen text-[#1A1C1E] font-['Barlow_Condensed',_sans-serif] overflow-x-hidden"
            ...
            <section ref={heroRef} className="relative overflow-hidden border-b border-gray-100" style={{ minHeight: 'auto', backgroundColor: '#fcfdfe' }}>
                ...
                <div className="relative z-10 max-w-[1600px] mx-auto px-6 lg:px-16 pt-24 lg:pt-32 pb-14">
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
                        <div className="max-w-2xl">
                            <div className="flex items-center gap-3 mb-5">
                                <div className="w-8 h-px bg-[#0061A4]" />
                                <span className="mono text-[#0061A4] text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.25em]">UzAuto Trailer — Fleet Catalog</span>
                                <Signal size={12} className="text-blue-500 animate-pulse" />
                            </div>
                            <h1 className="text-[48px] sm:text-[64px] lg:text-[92px] font-bold uppercase leading-[0.9] tracking-tight">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <button onClick={() => setSearchParams({ category: activeCategory, brand: 'all' })}
                                    className={cn("relative px-4 sm:px-6 py-2.5 sm:py-3 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest transition-all duration-300", activeBrand === 'all' ? "text-white bg-[#0061A4]" : "text-gray-400 border border-gray-100 hover:border-blue-500/40 hover:text-[#0061A4]")}>
                                ...
                                {brands.map((b) => (
                                    <button key={b.id} onClick={() => setSearchParams({ category: activeCategory, brand: b.id })}
                                        className={cn("relative px-4 sm:px-6 py-2.5 sm:py-3 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest transition-all duration-300", activeBrand === b.id ? "text-white bg-[#0061A4]" : "text-gray-400 border border-gray-100 hover:border-blue-500/40 hover:text-[#0061A4]")}>
                                        {b.name}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 sm:gap-8 mt-10 pt-6 border-t border-gray-100">
                        <div className="flex items-center gap-2">
                            <span className="mono text-[#0061A4] text-xl sm:text-2xl font-bold">{String(filteredProducts.length).padStart(2, '0')}</span>
                            <span className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest">{t('units_found')}</span>
                        </div>
                        <div className="hidden sm:block w-px h-6 bg-gray-100" />
                        <div className="flex items-center gap-2">
                            <span className="mono text-gray-400 text-xl sm:text-2xl font-bold">{String(categories.length).padStart(2, '0')}</span>
                            <span className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest">{t('categories_label')}</span>
                        </div>
                        <div className="flex-1 min-w-[20px] h-px bg-gradient-to-r from-blue-500/10 to-transparent" />
                        <button onClick={() => setSidebarOpen(true)} className="lg:hidden flex items-center gap-2 px-5 py-2.5 bg-[#0061A4] text-white text-[10px] font-bold uppercase tracking-widest hover:bg-blue-700 transition-all rounded-sm shadow-lg shadow-blue-100">
            ...
            <div className="max-w-[1600px] mx-auto px-6 lg:px-16 py-10 lg:py-14">
                <div className="flex flex-col lg:flex-row gap-12 items-start">
                    ...
                    {/* ASIDE RESPONSIVE TUZATILDI: h-full va overflow-y-auto qo'shildi */}
                    <aside className={cn(
                        "lg:w-[320px] shrink-0 lg:sticky lg:top-6 lg:border-r lg:border-gray-100 lg:pr-12 transition-transform duration-500 z-[70]",
                        sidebarOpen ? "fixed inset-y-0 left-0 w-[85vw] max-w-xs bg-white p-6 translate-x-0 shadow-2xl h-full overflow-y-auto" : "max-lg:-translate-x-full fixed inset-y-0 left-0 lg:static"
                    )}>
                        <div className="flex items-center justify-between mb-8 lg:mb-6 py-4 border-y border-gray-50">
                            ...
                            <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-2 hover:bg-gray-100 rounded-full">
                                <X size={18} />
                            </button>
                        </div>

                        {/* max-h-none qilinib ichki skroll cheklovi olib tashlandi */}
                        <div className="flex flex-col gap-1.5 scrollbar-hide">
                        ...
                        {/* 🛡️ FAQAT DESKTOPDA KO'RINADI (BU QISIM MENYUGA HALAQIT BERMAYDI) */}
                        <div className="hidden lg:block mt-10 relative overflow-hidden p-6" style={{ background: 'linear-gradient(135deg, rgba(0,97,164,0.1) 0%, rgba(255,255,255,1) 100%)', border: '1px solid #E5E7EB' }}>
                    ...
                    <main className="flex-1 min-w-0 w-full">
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-5">
                                <Loader2 className="text-[#0061A4] animate-spin" size={32} />
                                <p className="mono text-gray-400 text-[11px] tracking-[0.3em] uppercase">{t('loading_data')}</p>
                            </div>
                        ) : (
                                    ...
                                    {filteredProducts.map((p, idx) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col md:flex-row">
                                                <div className="relative w-full md:w-[320px] lg:w-[380px] shrink-0 overflow-hidden bg-gray-50 flex items-center justify-center p-6 sm:p-8 md:border-r border-gray-50 aspect-video md:aspect-auto">
                                                ...
                                                <div className="flex-1 flex flex-col p-6 sm:p-8 lg:p-10 min-w-0">
                                                    <div className="mb-6">
                                                        ...
                                                        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold uppercase tracking-tight text-[#0061A4] group-hover:text-blue-700 transition-colors">{getField(p, 'title')}</h2>
                                                    </div>

                                                    {p.techSpecs?.length > 0 && (
                                                        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-8">
                                                            {p.techSpecs.slice(0, 4).map((spec, i) => (
                                                                <div key={i} className="p-3 sm:p-3.5 bg-gray-50/80 border border-gray-100 rounded-xl min-h-[70px] sm:min-h-[80px] flex flex-col justify-start">
                                                                    <p className="mono text-gray-400 text-[8px] sm:text-[8.5px] tracking-widest uppercase mb-1 sm:mb-1.5 leading-tight">{getField(spec, 'key')}</p>
                                                                    <p className="text-[#1A1C1E] font-bold text-[11px] sm:text-[13px] uppercase tracking-tight leading-tight">{getField(spec, 'val')}</p>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    )}

                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mt-auto pt-6 border-t border-gray-50">
                                                        <div className="space-y-0.5">
                                                            <p className="mono text-gray-400 text-[9px] tracking-widest uppercase font-bold">{t('price')}</p>
                                                            <p className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tighter tabular-nums italic">
                                                                {p.price ? `${p.price} ${lang === 'ru' ? 'сум' : "so'm"}` : t('call_price')}
                                                            </p>
                                                        </div>
                                                        <button className="relative flex items-center justify-center gap-3 px-8 sm:px-10 py-4 sm:py-5 bg-[#0061A4] text-white rounded-xl sm:rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all hover:bg-blue-600 shadow-xl shadow-blue-100 active:scale-95">

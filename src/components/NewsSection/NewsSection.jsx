...
const NewsSection = () => {
    ...
    // RESPONSIVE SLIDER UCHUN STATE: Ekranga qarab siljish uzunligini belgilaydi
    ...
    // EKRAN O'ZGARGANIDA SLIDER QADAMINI HISOBLASH (Faqat shu logika qo'shildi)
    useEffect(() => {
        const updateWidth = () => {
            // Telefonda kartochka kengligi (280px) + gap (16px) = 296px
            // Kompyuterda kartochka kengligi (380px) + gap (24px) = 404px
    ...
    return (
        // Telefonda padding tepa-pastdan kichraytirildi (py-12 md:py-20 px-4 md:px-6)
        <section id="news-section" className="py-12 md:py-20 bg-white font-inter border-t border-gray-50 select-none">
            <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12 overflow-hidden">
                ...
                <div className="relative w-full cursor-grab active:cursor-grabbing">
                    <motion.div
                        // Telefonda kartalar orasi gap-4 (16px), kompyuterda gap-6 (24px)
                    ...
                    >
                        {infiniteNews.map((item, index) => (
                            // Telefonda min-w-[280px], kompyuterda min-w-[380px]
                            <div key={index} className="min-w-[280px] md:min-w-[380px] group bg-white rounded-[16px] md:rounded-[24px] border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
                                ...
                                {/* Rasm balandligi telefonda kichraytirilgan (h-40 md:h-56) */}
                                <Link to={`/news/${item.id}`} className="block relative h-40 sm:h-48 md:h-56 overflow-hidden shrink-0" onPointerDown={(e) => e.stopPropagation()}>
                                    <img src={`${API_URL}${item.image}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 pointer-events-none" alt="" />
                                </Link>
                                
                                {/* Matnlar qismi paddingi kichraytirildi (p-4 md:p-6) */}
                                <div className="p-4 md:p-6 flex flex-col flex-1">
                                    ...
                                    <Link to={`/news/${item.id}`} className="block mb-2" onPointerDown={(e) => e.stopPropagation()}>
                                        {/* Harf o'lchami telefonda text-base, kompyuterda text-lg */}
